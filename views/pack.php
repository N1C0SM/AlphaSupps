<?php require_once '../controllers/pack.php'; ?>
<!DOCTYPE html>
<html lang="es">
...
<div class="product-gallery">
  <div class="gallery-main">
    ...
    <img src="<?= $img ?>" id="main-product-image" width="1200" height="800" decoding="async" fetchpriority="high" loading="eager" alt="<?= htmlspecialchars($pack['name']) ?>">
...
<div class="product-info">
  <h1><?= htmlspecialchars($pack['name']) ?></h1>

  <!-- RATING -->
  <div class="product-rating" data-pack-id="<?= $pack['id'] ?>" data-can-rate="<?= $canRate ? '1' : '0' ?>">
    <?php for($i=1;$i<=5;$i++): ?>
      <button class="rating-star <?= $i <= $userRating?'active':'' ?>" data-value="<?= $i ?>" <?= $canRate ? '' : 'aria-disabled="true"' ?>>★</button>
    <?php endfor; ?>
    <div class="rating-meta">
      <span class="rating-value" id="pack-rating-value"><?= number_format($rating,1) ?></span>/5
      <small id="pack-rating-message"><?= $canRate ? '(Pulsa para valorar)' : 'Haz público el pack para permitir valoraciones' ?></small>
    </div>
  </div>
  ...
  <div class="purchase-buttons">
    <button class="cta-btn" onclick='addToCartWithSubscription({id:"pack_<?= $pack["id"] ?>",name:"<?= htmlspecialchars($pack["name"]) ?>",price:<?= $pack["price"] ?>,icon:"<?= $img ?>"})'>Añadir al carrito</button>
    <button class="cta-btn secondary" onclick='instantBuyWithSubscription({id:"pack_<?= $pack["id"] ?>",name:"<?= htmlspecialchars($pack["name"]) ?>",price:<?= $pack["price"] ?>,icon:"<?= $img ?>"})'>Comprar ahora</button>
  </div>
...
(() => {
  ...
  const ratingValueEl = document.getElementById('pack-rating-value');
  const messageEl = document.getElementById('pack-rating-message');
  const packId = parseInt(ratingBox.dataset.packId, 10);
  ...
  async function sendVote(val) {
    if (!canRate) {
      notify('Haz público el pack para permitir valoraciones.', 'error');
      return;
    }
